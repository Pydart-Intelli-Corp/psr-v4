...
export default function ComparisonSummary({ 
  ...
  const exportToCSV = () => {
    ...
    if (dailyData.length > 0) {
      ...
      csvContent.push('Date,Collections,Quantity (L),FAT (%),SNF (%),CLR,Amount (Rs),Rate (Rs/L)');
      dailyData.forEach((day) => {
        csvContent.push(`${day.date},${day.data.totalCollections},${day.data.totalQuantity.toFixed(2)},${day.data.weightedFat.toFixed(2)},${day.data.weightedSnf.toFixed(2)},${day.data.weightedClr.toFixed(2)},${day.data.totalAmount.toFixed(2)},${day.data.averageRate.toFixed(2)}`);
      });
      csvContent.push('');
    }
    ...
    csvContent = csvContent.concat([
      ...
      `${currentDate.from} to ${currentDate.to},${currentData.totalCollections},${currentData.totalQuantity.toFixed(2)},${currentData.weightedFat.toFixed(2)},${currentData.weightedSnf.toFixed(2)},${currentData.weightedClr.toFixed(2)},${currentData.totalAmount.toFixed(2)},${currentData.averageRate.toFixed(2)}`,
    ]);

    if (!isSamePeriod) {
      csvContent.push(`${previousDate.from} to ${previousDate.to},${previousData.totalCollections},${previousData.totalQuantity.toFixed(2)},${previousData.weightedFat.toFixed(2)},${previousData.weightedSnf.toFixed(2)},${previousData.weightedClr.toFixed(2)},${previousData.totalAmount.toFixed(2)},${previousData.averageRate.toFixed(2)}`);
      csvContent.push(`Difference,${(currentData.totalCollections - previousData.totalCollections).toFixed(0)},${(currentData.totalQuantity - previousData.totalQuantity).toFixed(2)},${(currentData.weightedFat - previousData.weightedFat).toFixed(2)},${(currentData.weightedSnf - previousData.weightedSnf).toFixed(2)},${(currentData.weightedClr - previousData.weightedClr).toFixed(2)},${(currentData.totalAmount - previousData.totalAmount).toFixed(2)},${(currentData.averageRate - previousData.averageRate).toFixed(2)}`);
    }
  ...
  const exportToPDF = () => {
    ...
    if (dailyData.length > 0) {
      ...
      autoTable(doc, {
        startY: startY + 4,
        head: [['Date', 'Collections', 'Qty (L)', 'FAT (%)', 'SNF (%)', 'CLR', 'Amt (Rs)', 'Rate (Rs/L)']],
        body: dailyRows,
        theme: 'grid',
    ...
    const summaryBody = isSamePeriod 
      ...
      : [
          [`${currentDate.from} to ${currentDate.to}`, currentData.totalCollections, currentData.totalQuantity.toFixed(2), currentData.weightedFat.toFixed(2), currentData.weightedSnf.toFixed(2), currentData.weightedClr.toFixed(2), currentData.totalAmount.toFixed(2), currentData.averageRate.toFixed(2)],
          [`${previousDate.from} to ${previousDate.to}`, previousData.totalCollections, previousData.totalQuantity.toFixed(2), previousData.weightedFat.toFixed(2), previousData.weightedSnf.toFixed(2), previousData.weightedClr.toFixed(2), previousData.totalAmount.toFixed(2), previousData.averageRate.toFixed(2)],
    ...
    autoTable(doc, {
      startY: startY + 4,
      head: [['Period', 'Total Collections', 'Total Quantity (L)', 'Weighted FAT (%)', 'Weighted SNF (%)', 'Weighted CLR', 'Total Amount (Rs)', 'Avg Rate (Rs/L)']],
      body: summaryBody,
      theme: 'grid',
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  // Reset BMC and Society when dairy changes
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  // Reset Society when BMC changes
                  if (onSocietyChange) onSocietyChange([]);
                }}
                className="px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-emerald-500"
              >
                ...
                {bmcs
                  .filter(bmc => {
                    // If dairy is selected, show only BMCs under that dairy
                    if (dairyFilter.length > 0) {
          ...
          {onSocietyChange && societies.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {societies
                  .filter(society => {
                    // If BMC is selected, show only societies under that BMC
                    if (bmcFilter.length > 0) {
                      const selectedBmcId = parseInt(bmcFilter[0]);
                      return society.bmc_id === selectedBmcId;
                    }
                    // If only dairy is selected, show societies under BMCs of that dairy
                    if (dairyFilter.length > 0) {
                      const selectedDairyId = parseInt(dairyFilter[0]);
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {dailyData.map((day, index) => {
                ...
                return (
                  <tr key={day.date} className={isEven ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-gray-700/50'}>
                    ...
                    <td className="px-4 py-3 text-center text-sm text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600">
                      <span className="font-semibold">{day.data.totalCollections}</span>
                    </td>
                    <td className="px-4 py-3 text-center text-sm text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600">
                      <span className="font-semibold text-emerald-600 dark:text-emerald-300">{day.data.totalQuantity.toFixed(2)}</span>
                    </td>
                    <td className="px-4 py-3 text-center text-sm text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600">
                      <span className="font-semibold text-amber-600 dark:text-amber-300">{day.data.weightedFat.toFixed(2)}</span>
                    </td>
                    <td className="px-4 py-3 text-center text-sm text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600">
                      <span className="font-semibold text-purple-600 dark:text-purple-300">{day.data.weightedSnf.toFixed(2)}</span>
                    </td>
                    <td className="px-4 py-3 text-center text-sm text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600">
                      <span className="font-semibold text-pink-600 dark:text-pink-300">{day.data.weightedClr.toFixed(2)}</span>
                    </td>
                    <td className="px-4 py-3 text-center text-sm text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600">
                      <span className="font-semibold text-blue-600 dark:text-blue-300">₹{day.data.totalAmount.toFixed(2)}</span>
                    </td>
                    <td className="px-4 py-3 text-center text-sm text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600">
                      <span className="font-semibold text-cyan-600 dark:text-cyan-300">₹{day.data.averageRate.toFixed(2)}</span>
                    </td>
                  </tr>
                );
              })}
              ...
              <tr className="bg-gradient-to-r from-emerald-100 to-cyan-100 dark:from-emerald-900/50 dark:to-cyan-900/50 font-bold">
                ...
                <td className="px-4 py-3 text-center text-sm text-emerald-700 dark:text-emerald-200 border border-gray-300 dark:border-gray-600">
                  {currentData.totalQuantity.toFixed(2)}
                </td>
                <td className="px-4 py-3 text-center text-sm text-amber-700 dark:text-amber-200 border border-gray-300 dark:border-gray-600">
                  {currentData.weightedFat.toFixed(2)}
                </td>
                <td className="px-4 py-3 text-center text-sm text-purple-700 dark:text-purple-200 border border-gray-300 dark:border-gray-600">
                  {currentData.weightedSnf.toFixed(2)}
                </td>
                <td className="px-4 py-3 text-center text-sm text-pink-700 dark:text-pink-200 border border-gray-300 dark:border-gray-600">
                  {currentData.weightedClr.toFixed(2)}
                </td>
                <td className="px-4 py-3 text-center text-sm text-blue-700 dark:text-blue-200 border border-gray-300 dark:border-gray-600">
                  ₹{currentData.totalAmount.toFixed(2)}
                </td>
                <td className="px-4 py-3 text-center text-sm text-cyan-700 dark:text-cyan-200 border border-gray-300 dark:border-gray-600">
          ...
          <tbody>
            ...
            <tr className="bg-blue-50 dark:bg-blue-900/20">
              <td className="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white border border-gray-300 dark:border-gray-600">
                {currentDate.from} to {currentDate.to}
              </td>
            ...
            {(currentDate.from !== previousDate.from || currentDate.to !== previousDate.to) && (
              <>
                <tr className="bg-green-50 dark:bg-green-900/20">
                  <td className="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white border border-gray-300 dark:border-gray-600">
                    {previousDate.from} to {previousDate.to}
                  </td>
                  <td className="px-4 py-3 text-center text-sm text-gray-900 dark:text-white border border-gray-300 dark:border-gray-600">
      ...
      <div className="mt-6 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        ...
        {(() => {
          // Normalize function: converts actual value to 0-100 scale based on min/max
          ...
          <div className="bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm rounded-xl p-4 shadow-inner border border-gray-200/50 dark:border-gray-700/50">
            <ResponsiveContainer width="100%" height={450}>
              <AreaChart data={dailyData.map(d => {
                // Calculate min/max for normalization across all days
